...
Optionally, if we need to generate additional access keys for the ~$FUNDED_ACCOUNT~, run the following command N times. Note that this will generate keys for implicit accounts, but we'll then tie them to our ~example.near~ account.
